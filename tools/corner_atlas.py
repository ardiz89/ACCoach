...
HELD: dict[str, str] = {
    "budapest":
        ...
        "the 17 apexes against 14 turns is just the merging, as at Sepang. Held "
        "since 2026-08-04 because two guides contradict each other and the "
        "geometry could not arbitrate; re-attacked on 2026-08-05 with the "
        "method that had just freed Montreal, and it came back **a firmer no**. "
        "\n\n"
        "A third guide was found and read, one that states nine of the fourteen "
        "directions (T2-T5 'alternating left-right-left-right', 'Turn 10, a "
        "right-hander', T12-T14 'a rhythmic right-left-right') plus a "
        "structural anchor of its own: the straight between T11 and T12 is the "
        "second-longest of the lap. That list plus that anchor cuts the "
        "admissible readings to **16, and all sixteen agree on T4 through "
        "T12** — nine consecutive rows identical. So the disagreement is now "
        "narrow. It also sits exactly where it hurts. "
        "\n\n"
        "**What settles it, and it is a fact about our own tooling rather than "
        "about the circuit.** A fourth guide (Coach Dave Academy, ACC-facing) "
        "calls T10 'an easy flat-out **left**-hand kink' where the other calls "
        "it a right-hander, and both survive the road. Worse: at the default "
        "220 m ceiling the right-hander reading has **zero** admissible "
        "assignments while the left-hander reading has 18; at 300 m they have "
        "48 and 108. Which guide 'wins' therefore moves when we move a knob of "
        "ours. A circuit whose answer depends on the detector's setting is not "
        "measured, it is tuned — and the previous note had the right verdict "
        "for a reason that was too weak (it blamed five unstated directions; "
        "nine stated ones do not fix it either).",
    "sochi":
        "Right layout (5836 m against 5848). Held since 2026-08-04 on a "
        "tie-break between two sources; re-attacked 2026-08-05 and the reason "
        "is now a **number** instead of an anecdote. "
        "\n\n"
        "The global constraint — 12 rights and 6 lefts, the lever that freed "
        "Catalunya and that Melbourne predicted before reading — is not enough "
        "here, and it is worth seeing why. The trace gives 24 apexes in nine "
        "same-direction runs: rights 2-4-3-1-3 (13 apexes for 12 turns, so a "
        "single merge) and lefts 4-2-2-3 (**11 apexes for 6 turns**, so five). "
        "Merging only adjacent apexes of the same hand, the number of readings "
        "that satisfy 12R/6L is **168**. At Catalunya that same constraint left "
        "one. The difference is that Sochi's lefts are long and re-read: the "
        "run of four between 0.248 and 0.318 is 410 m of one corner, the famous "
        "180-degree left. "
        "\n\n"
        "And the two sources still put that corner at different numbers (T3 "
        "against T4) — verified again on 2026-08-05, RaceFans states verbatim "
        "'Its most distinctive corner, turn four, is a 180-degree left-hand "
        "turn'. One of the sources also describes a 'Turn 19' on an "
        "eighteen-turn circuit. "
        "\n\n"
        "One more warning, and it is about the trace rather than the sources: "
        "RaceFans publishes 'Distance from grid to turn one 890.6 m', and the "
        "nearest apex to that point sits **295 m** away (there are apexes at "
        "445 m and 1185 m, nothing between). That is larger than any "
        "frame offset measured on a curated circuit (12-116 m), so either this "
        "trace's origin is not where the others' is, or turn one is not a "
        "curvature feature at all. Either way, nothing here should be anchored "
        "by distance until that is understood.",
    "yasmarina":
