...
def test_progress_does_not_serve_tyres_over_the_whole_archive(tmp_path):
    """Trends used to build a per-lap tyre series and label it "across the
    stint". The span was every lap ever recorded for the car and track — three
    evenings, three track temperatures, any number of refuels in between — so
    the heading and the data disagreed. It lives on /api/stint now, which has a
    stint. Kept as a test because a payload nobody reads grows back."""
